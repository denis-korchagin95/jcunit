    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (dirent->d_type == DT_DIR) {
                ...
                struct path_list * new_directory = memory_blob_pool_alloc(&memory_pool, sizeof(struct path_list));
...
char * get_entry_path(const char * path, uint32_t path_len, struct dirent * dirent, unsigned int flags)
{
    ...
    char * entry_path = memory_blob_pool_alloc(&memory_pool, entry_len + 1);
